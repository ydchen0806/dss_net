...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\linewidth]{figs/flowchart.eps}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\linewidth]{figs/channel_mechanisms.eps}
...
{
	\includegraphics[width=0.45\linewidth]{figs/Bellhop_SSP.eps}
}
	\subfloat[\label{fig:All_channel}]
	{
		\includegraphics[width=0.45\linewidth]{figs/All_channel.eps}
	}
	\\
	\subfloat[\label{fig:Static_channel}]
	{
		\includegraphics[width=0.45\linewidth]{figs/Static_channel.eps}
	}
	\subfloat[\label{fig:Dynamic_channel}]
	{
		\includegraphics[width=0.45\linewidth]{figs/Dynamic_channel.eps}
	}
...
\begin{figure*}[!htbp]
	...
	\subfloat[\label{fig:WuyuanBay}]{
		\includegraphics[width=0.25\textwidth]{figs/WuyuanBay_CH11.eps}
	}
	\subfloat[\label{fig:CH11}]{
		\includegraphics[width=0.24\textwidth]{figs/WuyuanBay_CH11.eps}
	}
	\subfloat[\label{fig:CH12}]{
		\includegraphics[width=0.24\textwidth]{figs/WuyuanBay_CH12.eps}
	}
	\subfloat[\label{fig:CH13}]{
		\includegraphics[width=0.24\textwidth]{figs/WuyuanBay_CH13.eps}
	}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\linewidth]{figs/Seatrial_BER.eps}
...
\begin{figure*}[!htbp]
	...
	\subfloat[Conventional]
	{
		...
		\includegraphics[width=0.24\textwidth]{figs/est_QPSK_GAON.eps}
	}
	\subfloat[DSS-Net]
	{
		\label{fig:scatterplot_DSSNet}
		\includegraphics[width=0.24\textwidth]{figs/est_QPSK_GIAON.eps}
	}
...
\par Since ground-truth channel matrices are unavailable in real environments, we employ alternative metrics to evaluate DSS-Net performance. The \textit{denoising gain} quantifies the noise energy removed from the channel:
\begin{equation}
	\text{Gain}_{\text{dB}} = 10\log_{10}\frac{P_{\text{input}}}{P_{\text{output}}},
\end{equation}
where $P_{\text{input}}$ and $P_{\text{output}}$ denote the power of input (noisy) and output (denoised) channels, respectively. The \textit{estimated SNR improvement} is computed as:
\begin{equation}
	\Delta\text{SNR}_{\text{est}} = 10\log_{10}\frac{P_{\text{output}}}{P_{\text{input}} - P_{\text{output}}}.
\end{equation}
...
\begin{itemize}
	...
	\item \textbf{Consistent denoising performance}: DSS-Net achieves positive denoising gains (1.31--2.39~dB) across all depths, demonstrating robust generalization to unseen real-world channels.
...
\begin{table}[htbp]
	\centering
	\caption{\textsc{Fuxian Lake Sea Trial Results.} Performance metrics of DSS-Net evaluated on real channel measurements at different receiver depths. Denoising gain and estimated SNR improvement quantify the noise reduction achieved. Static/Dynamic ratios reflect the learned channel decomposition.}
	\label{tab:fuxian_results}
	\renewcommand{\arraystretch}{1.15}
	\setlength{\tabcolsep}{4pt}
	\small
	\begin{tabular}{>{\centering\arraybackslash}p{1.2cm} >{\centering\arraybackslash}p{1.4cm} >{\centering\arraybackslash}p{1.5cm} >{\centering\arraybackslash}p{1.3cm} >{\centering\arraybackslash}p{1.3cm}}
		\toprule[1.5pt]
		\rowcolor{tableheader}
		\textbf{Depth} & \textbf{Denoising} & \textbf{Est. SNR} & \textbf{Static} & \textbf{Dynamic} \\
		\rowcolor{tableheader}
		\textbf{(m)} & \textbf{Gain (dB)} & \textbf{Improv. (dB)} & \textbf{Ratio} & \textbf{Ratio} \\
		\midrule[1pt]
		\rowcolor{tablerow1}
		5 & 2.39 & 4.99 & 69.3\% & 23.8\% \\
		\rowcolor{tablerow2}
		7 & 1.38 & 7.29 & 65.3\% & 24.6\% \\
		\rowcolor{tablerow1}
		9 & 1.31 & 7.46 & 52.8\% & 29.0\% \\
		\bottomrule[1.5pt]
	\end{tabular}
	\renewcommand{\arraystretch}{1.0}
\end{table}
