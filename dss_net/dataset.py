...
class ChannelDataset(Dataset):
    ...
    def __init__(
        ...
        pattern: str = "/LSEM/user/chenyinda/code/signal_xiaoyu/2D_channel_for_DSS/Tx_all_2D_*.mat",
        mode: str = "train",
...
def create_dataloaders(
    ...
    Args:
        ...
        rank: DDP rank（默认0）
        world_size: 总进程数（默认1）
        use_ddp: 是否使用DDP（只有world_size>1且配置启用时才为True）
    Returns:
    ...
    # 🔧 关键修复：只在真正使用DDP时才使用DistributedSampler
    if use_ddp:
        ...
        shuffle_train = False  # DistributedSampler会处理shuffle
        ...
        if rank == 0:
            print(f"✅ Using standard DataLoader (single GPU)")
    
    # 🔧 内存优化：强制num_workers=0避免fork导致的内存爆炸
    num_workers = 0
    if rank == 0:
        print("\n⚠️  内存优化模式：num_workers=0 (避免fork内存复制)")
